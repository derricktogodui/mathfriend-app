...
def get_typing_users():
    try:
        ...
        # Return users who are typing, excluding the current user
    ...
    while len(options) < 4:
        try:
            ...
            if numeric_part_str:
                ...
                # Format based on original answer's precision
...
def format_message(message, mentioned_usernames, current_user):
    ...
    # Make a copy to avoid modifying the original message
    ...
    # Convert markdown-style bold/italics to HTML
...
def show_main_app():
    ...
    with st.sidebar:
        ...
        # Display online users in the sidebar
        ...
        if online_users:
            ...
            st.write("No other users are online.")
        ...
        if top_scores:
            ...
            for rank, (username, score, total) in enumerate(top_scores, 1):
                ...
                leaderboard_data.append({
                    "Rank": f"#{rank}", "Username": username, "Score": f"{score}/{total}", "Accuracy": f"{accuracy:.1f}%"
                })
            
            df = pd.DataFrame(leaderboard_data)
            
            def highlight_user(row):
                return ['background-color: #e6f7ff; font-weight: bold;'] * len(row) if row.Username == st.session_state.username else [''] * len(row)
    ...
    # --- ###################################################### ---
    # --- ### FULLY INTEGRATED CHAT SECTION ### ---
    # --- ###################################################### ---
    elif selected_page == "💬 Chat":
        st.header("💬 Community Chat")
        st_autorefresh(interval=3000, key="chat_refresh") # Refresh every 3 seconds

        # Get chat messages and usernames
        ...
        with chat_container:
            ...
            for msg in all_messages:
                is_own_message = msg['username'] == st.session_state.username
                
                col1, col2 = st.columns([1, 8]) if not is_own_message else st.columns([8, 1])
                
                with col1 if not is_own_message else col2:
                    st.image(get_avatar_url(msg['username']), width=40)
                
                with col2 if not is_own_message else col1:
                    row_class = "own" if is_own_message else "other"
                    st.markdown(f"""
        ...
        if typing_users:
            indicator_placeholder.markdown(f"<div class='typing-indicator'>*{typing_text} is typing...*</div>", unsafe_allow_html=True)
        
        # Chat input form
        with st.form("chat_form", clear_on_submit=True):
            col1, col2 = st.columns([4, 1])
            with col1:
                user_message = st.text_area("Your message:", key="chat_input", height=75, placeholder="Type a message... Use @ to mention a user.", on_change=update_typing_status, args=(st.session_state.username, True))
            with col2:
                uploaded_file = st.file_uploader("Upload Image", type=["png", "jpg", "jpeg", "gif"], label_visibility="collapsed")
