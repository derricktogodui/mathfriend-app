...
def display_duel_page():
    ...
    # --- THIS IS THE KEY FIX ---
    # The "Game Over" logic is now the FIRST thing the function checks.
    # It uses the duel's 'status' from the database as the definitive signal that the game has ended.
    if status != 'active':
        st.balloons()
        winner_username = ""
        if status == 'player1_win': winner_username = player1
        ...
        # Display the final progress bar, capped correctly at 10/10.
        st.progress(1.0, text="Question 10/10")
        ...
        # The 'return' is crucial. It stops any other code below from running, preventing the refresh.
        return

    # --- The rest of the function now only runs if the game is still active ---
