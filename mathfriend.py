...
def show_login_page():
    ...
    with col2:
        ...
        /* Dark mode extension */
        .dark .chat-container { background:#111 !important; border-color:#333 !important; }
        .dark .content-card { background:#121212 !important; color:#f0f0f0 !important; border-color:#333 !important; }
        .dark .msg-row.msg-other .msg-bubble { background:#333 !important; color:#fff !important; }
        .dark .msg-row.msg-own .msg-bubble { background:#0d47a1 !important; color:#fff !important; }
        .dark .sidebar, .dark .stSidebar { background:#0e0e0e !important; color:#f0f0f0 !important; }
        .dark input, .dark textarea { background:#1b1b1b !important; color:#f0f0f0 !important; border-color:#333 !important; }
        
        .brand-bar { position: sticky; top:0; z-index: 5; background: var(--light); padding: 8px 12px; border-bottom: 1px solid rgba(0,0,0,0.06); }
        .dark .brand-bar { background:#111; border-color:#333; }
        </style>
            ...
            with st.form("login_form"):
                ...
                if submitted:
                    if login_user(username, password):
                        ...
                        st.experimental_set_query_params(mf='1', u=username)
...
def show_main_app():
    ...
    # Main content container
    st.markdown("<div id='main_brand_header' class='brand-bar'><h2 style='margin:0'>🧮 MathFriend</h2></div>", unsafe_allow_html=True)
    ...
    <div style="display: flex; align-items: center; margin-bottom: 20px;">
        <div style="position: relative; display: inline-block;">
            <img src="{avatar_url}" style="width: 60px; height: 60px; border-radius: 50%; margin-right: 15px; border: 3px solid #4361ee; object-fit: cover;"/>
...
# [Previous imports and code remain exactly the same until the Chat section]
        ...
        if "chat_visible_count" not in st.session_state:
            st.session_state.chat_visible_count = 40  # start with last 40
        
        total_msgs = len(all_messages)
        start_idx = max(0, total_msgs - st.session_state.chat_visible_count)
        messages_to_show = all_messages[start_idx:]
        
        cols_lazy = st.columns([0.5,0.5])
        with cols_lazy[0]:
            if start_idx > 0 and st.button("⬆️ Load older", key="load_older"):
                st.session_state.chat_visible_count = min(total_msgs, st.session_state.chat_visible_count + 40)
                st.rerun()
        with cols_lazy[1]:
            if st.button("⬇️ Jump to bottom", key="jump_bottom"):
                st.session_state.chat_visible_count = 40 if total_msgs<=40 else 40
                st.experimental_set_query_params(_scroll="bottom")
                st.rerun()
...
# Restore session from query params to prevent logout on refresh
params = st.experimental_get_query_params()
if not st.session_state.get("logged_in") and params.get("mf") == ['1'] and params.get("u"):
    st.session_state.logged_in = True
    st.session_state.username = params.get("u")[0]
